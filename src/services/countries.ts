...
const countries = {
    ...
    getCountry: (countryCode: string) => {
        const res = _.find(countries.list, country => {
    ...
    getCountryCodeFromIP: (ip?: string) => new Promise((resolve, reject) => {
        let uri = "http://ip-api.com/json"
        if (ip) {
            uri = `${uri}/ip`
        }
        ...
        axios.get(uri).then(res => {
            if (res.data.countryCode) {
                return resolve(res.data.countryCode)
            } else {
                // If it can't be retrieved, just set a random one
                return reject(_.shuffle(countries.list)[0].code)
            }
        }).catch(err => {
            // If it can't be retrieved, just set a random one
            return reject(_.shuffle(countries.list)[0].code)
